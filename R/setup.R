...
update_env_to_equ <- function(num_time_steps, pop,
  ...
  JuliaCall::julia_assign("human_cercariae", pop[[4]])
  JuliaCall::julia_assign("female_worms", pop[[8]])
  JuliaCall::julia_assign("male_worms", pop[[9]])
  JuliaCall::julia_assign("time_step", time_step)
  JuliaCall::julia_assign("average_worm_lifespan", average_worm_lifespan)
  JuliaCall::julia_assign("eggs", pop[[5]])
  JuliaCall::julia_assign("max_fecundity", max_fecundity)
  JuliaCall::julia_assign("r", r)
  JuliaCall::julia_assign("worm_stages", worm_stages)
  JuliaCall::julia_assign("vac_status", pop[[6]])
  JuliaCall::julia_assign("gender", pop[[2]])
  JuliaCall::julia_assign("predis_aggregation", predis_aggregation)
  JuliaCall::julia_assign("predisposition", pop[[3]])
  JuliaCall::julia_assign("treated", pop[[7]])
  ...
  JuliaCall::julia_assign("death_rate", pop[[10]])
...
run_repeated_sims_no_population_change <- function(num_repeats, num_time_steps,
  ...
  outputs = JuliaCall::julia_eval("run_repeated_sims_no_population_change(num_repeats, num_time_steps,
                                       time_step, average_worm_lifespan,
                                       max_fecundity, r, worm_stages, predis_aggregation, predis_weight,
                                       vaccine_effectiveness,
                                       density_dependent_fecundity, contact_rate, env_cercariae_survival_prop, env_miracidia_survival_prop,
                                       female_factor, male_factor, contact_rates_by_age,
                                       death_rate_per_time_step, birth_rate, mda_info, vaccine_info, mda_adherence, mda_access,
...
save_file_in_julia <- JuliaCall::julia_eval('
function save_file_in_julia(filename,  ages , gender,  predisposition,
    human_cercariae,  eggs,
    vac_status, treated, female_worms, male_worms,
    vaccinated,  age_contact_rate, death_rate,
    env_miracidia, env_cercariae, adherence, access)                                        
...
load_population_from_file <- function(filename, N){
  ...
  result <- JuliaCall::julia_eval("load_population_from_file(filename, N, true)")
}
...
plot_data_from_julia <- function(x, pop, filename, ytitle="", xtitle = ""){
  ...
  plot(times, sac_prev,type = 'l', col = 'salmon', ylim = c(0,100),bty = 'n', ylab = ytitle, xlab = xtitle)
  lines(times, high_burden_sac, col = 'cornflowerblue',type = 'l',ylim = c(0,100))
  
  
  save_file_in_julia(filename,  ages_equ , gender_equ,  predisposition_equ,
                     human_cercariae_equ,  eggs_equ,
                     vac_status_equ, treated_equ, female_worms_equ, male_worms_equ,
                     vaccinated_equ,  age_contact_rate, death_rate,
                     env_miracidia_equ, env_cercariae_equ, adherence, access)     
